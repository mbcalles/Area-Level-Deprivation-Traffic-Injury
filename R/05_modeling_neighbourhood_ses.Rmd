...
pd_cc_mi <- moran.test(cma_da$n_pedestrian_casualty_claims, listw)
pd_cc_mi  # Print results for pedestrian casualties

extract_moran_stats <- function(moran_result) {
  tibble(
    morans_i = round(moran_result$estimate["Moran I statistic"], 3),
    z_score = round(moran_result$statistic, 2),
    p_value = ifelse(moran_result$p.value < 0.001, 
                     "<0.001", 
...
morans_table <- bind_rows(
  ...
  # Reorder columns
  select(casualty_type, morans_i, z_score, p_value) %>%
...
Spatial autocorrelation analysis showed that all types of casualty claims were spatially clustered. Cyclist casualty claims had the highest spatial dependence (Moran's I = 0.372, z = 49.92, p < 0.001), followed by pedestrian casualty claims (Moran's I = 0.291, z = 38.84, p < 0.001) and all casualty claims (Moran's I = 0.249, z = 33.29, p < 0.001). 
...
ggsave(
  device = "svg",
  filename = "C:/Users/micha/Documents/GitHub/Area-Level-Deprivation-Traffic-Injury/Figures/study_area.svg",
  plot = study_area,
  units = "mm",
  height = 140*1.1,
  width = 190*1.1,
...
```{r figure 2, fig.height=12.75591,fig.width=10.09843}
...
vandix_distribution <-  cma_da_tibble %>%
   ...
   hrbrthemes::theme_ipsum_es(axis_title_size = 12,
                              grid = "XxYy",
                              ticks = TRUE) +
...
captions <- c(
  ...
  "Presence of Bike Infrastructure within 500m of dissemination area centroid from Canadian Bikeway Comfort and Safety Classification (CanBICS)",
  "Population",
...
create_interactive_map <- function(cma_da) {
  ...
  m <- leaflet(cma_da_wgs84) %>%
    ...
    addLegend(
      ...
      pal = pal_region,
      # Changed from pal_pop to pal_region
      values = ~ region,
      # Changed from ~pal_region to ~region
      title = "Region",
      opacity = 0.9,
      group = "Region"
    )
  ...
  m <- m %>%
    ...
    addLegend(
      ...
      title = "BYM2 Random Effect<br>(All Injuries)",
      opacity = 0.7,
      group = "BYM2 Random Effect - All Injuries"
    ) %>%
    # Cyclist model
    ...
    addLegend(
      ...
      title = "BYM2 Random Effect<br>(Cyclist Injuries)",
      opacity = 0.7,
      group = "BYM2 Random Effect - Cyclists"
    ) %>%
    # Pedestrian model
    ...
    addLegend(
      ...
      title = "BYM2 Random Effect<br>(Pedestrian Injuries)",
      opacity = 0.7,
      group = "BYM2 Random Effect - Pedestrians"
    )
  ...
  m <- m %>%
    addLayersControl(
      ...
      options = layersControlOptions(collapsed = FALSE, groupCheckboxes = TRUE)
    ) %>%
...
saveWidget(
  map,
  "C:/Users/micha/Documents/GitHub/Area-Level-Deprivation-Traffic-Injury/Supplementary Material/Supplementary Material 1.html",
  selfcontained = TRUE,
  title = "Spatial Distribution of Model Inputs and Outputs"
)
```
...
## Model Comparison of Fit Metrics
