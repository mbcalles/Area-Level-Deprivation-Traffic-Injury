...
dra <- #road network polyline
  st_read(dsn = paste0(getwd(), "/Processed Data/dra_subset.gpkg"))
...
$\eta_i = \beta_0 + \beta_1\text{vandix}_i + \sum_{k=1}^8\beta_{2k}\text{region}_{ki} + \sum_{k=1}^8\beta_{3k}(\text{vandix}_i \times \text{region}_{ki}) + \mathbf{X}_i\boldsymbol{\gamma} + u_i$

The model includes 16 regional indicators ($\text{region}_{ki}$) with Vancouver-Squamish serving as the reference category. The coefficient $\beta_{3k}$ represents the region-specific effect of the Vancouver Area Disadvantage Index (VanDIX), capturing how the relationship between disadvantage and casualty claims varies by region.
...
To handle disconnected components in the spatial structure, we employed sum-to-zero constraints for each region separately, using the adjust.for.con.comp = TRUE option in INLA. This approach assumes a common intercept across disconnected regions, with spatial random effects interpreted as area-specific deviations from the overall risk within each region. The model was scaled with respect to each subgraph to ensure comparability of random effects across regions.
...
fun <- function() return (exp(vandix_z + `vandix_z:regionVictoria`))
vic <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionOkanagan`))
ok <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionCentral Island`))
ci <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionNortheast`))
ne <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionKamloops`))
ka <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionCampbell River`))
cr <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionParksville`))
pa <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionCranbrook`))
cb <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionSquamish`))
sq <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionTerrace`))
te <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionSalmon Arm`))
sa <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionNelson`))
ne  <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionPowell River`))
pri  <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionTrail`))
tr  <- inla.posterior.sample.eval(fun, m.samp)

fun <- function() return (exp(vandix_z + `vandix_z:regionPrince Rupert`))
pr  <- inla.posterior.sample.eval(fun, m.samp)


vandix_irr_by_region <- tibble(region = region_levels,
                               vandix_irr_post = list(van, 
                                                      vic,
                                                      ok,
                                                      ci,
                                                      ne,
...
results_hyper_parameters <- lapply(inla_model_list, function(x) {
  if (!is.null(x$summary.hyper) && nrow(x$summary.hyper) > 0) {
    x$summary.hyper %>% rownames_to_column(var = "term")
  } else {
    # Create a single row data frame with NA values
    tibble(
      term = NA,
      mean = NA,
      sd = NA,
...
model_names <- c("Poisson",
                 "ZI Poisson",
                 "Poisson - Unstructured RE",
                 "ZI Poisson - Unstructured RE",
                 "Poisson - Spatial + Unstructured RE",
                 "ZIPoisson  - Spatial + Unstructured RE"
                      )
...
all_crashes_full_model_details %>%  
  filter(str_detect(term,"WAIC|DIC")) %>% 
  select(term,contains("mean")) %>% 
  pivot_longer(cols = contains("Poisson")) %>% 
  pivot_wider(id_cols = name,names_from = term,values_from = value) %>% 
  arrange(WAIC)


#Best fitting model is bym2

#######  Adjusted model - 

f4_all <-  n_casualty_claims ~ vandix_z*region + ln_roads_km_c + f(ui, model = 'bym2', graph = cma_adj, hyper = prior, adjust.for.con.comp = TRUE, constr = TRUE, scale.model = TRUE)
...
f5_all <-  n_casualty_claims ~ vandix_z*region + ln_roads_km_c +  roads_prop_highway_arterial_z +  bike_infra_cat + population_100_c + number_intersections_10_c  + f(ui, model = 'bym2', graph = cma_adj, hyper = prior, adjust.for.con.comp = TRUE, constr = TRUE, scale.model = TRUE)
...
all_vandix_irr_unadjusted <- inla.posterior.sample(1000, m3_all)
all_vandix_irr_minimally_adjusted <- inla.posterior.sample(1000, m4_all)
all_vandix_irr_adjusted <- inla.posterior.sample(1000, m5_all)
...
vandix_irr_all_crashes_models <- left_join(all_vandix_irr_unadjusted,all_vandix_irr_minimally_adjusted,by="region") %>% 
  left_join(all_vandix_irr_adjusted,by="region") %>% 
  mutate(Term = "VanDIX (SD)") %>% 
  select(Term,region,contains("IRR (95% CI)")) %>% 
  flextable() %>%
  add_header_row( values = c("","Incidence Rate Ratio (95% Credible Interval)"),colwidths = c(2,3)) %>% 
    set_header_labels(values = c("Term","Region","Unadjusted","Minimally Adjusted","Adjusted")) %>% 
    footnote(
      i = c(2),
      j = c(3, 4, 5),
      value = as_paragraph(
        c("Poisson model fit with idd and spatial random effect at dissemination area level",
...
model_list_cyclist <- list(m1_cyclist,m1_cyclist_zip,m2_cyclist,m2_cyclist_zip,m3_cyclist, m3_cyclist_zip)
model_names <- c("Poisson",
                 "ZI Poisson",
                 "Poisson - Unstructured RE",
                 "ZI Poisson - Unstructured RE",
...
cyclist_crashes_full_model_details <- summarise_models_details(inla_model_list = model_list_cyclist,inla_model_names = model_names )


cyclist_crashes_full_model_details %>%  
  filter(str_detect(term,"WAIC|DIC")) %>% 
  select(term,contains("mean")) %>% 
  pivot_longer(cols = contains("Poisson")) %>% 
  pivot_wider(id_cols = name,names_from = term,values_from = value) %>% 
...
f4_cyclist <-  n_cyclist_casualty_claims ~ vandix_z*region + ln_roads_km_c + f(ui, model = 'bym2', graph = cma_adj, hyper = prior, adjust.for.con.comp = TRUE, constr = TRUE, scale.model = TRUE)

m4_cyclist_zip <- inla(f4_cyclist,
           data =  cma_sp@data,
           family = "zeroinflatedpoisson1",
           control.compute = list(dic = TRUE, waic = TRUE,config=TRUE),
           control.predictor = list(compute = TRUE),
...
f5_cyclist <-  n_cyclist_casualty_claims ~ vandix_z*region + ln_roads_km_c +  roads_prop_highway_arterial_z +  bike_infra_cat + population_100_c + number_intersections_10_c  + f(ui, model = 'bym2', graph = cma_adj, hyper = prior, adjust.for.con.comp = TRUE, constr = TRUE, scale.model = TRUE)

m5_cyclist_zip <- inla(f5_cyclist,
           data =  cma_sp@data,
           family = "zeroinflatedpoisson1",
           control.compute = list(dic = TRUE, waic = TRUE,config=TRUE),
           control.predictor = list(compute = TRUE),
...
######### Region Specific IRRs from Posterior Distribution of Vandix + Region Interaction
cyclist_vandix_irr_unadjusted <- inla.posterior.sample(1000, m3_cyclist_zip)
cyclist_vandix_irr_minimally_adjusted <- inla.posterior.sample(1000, m4_cyclist_zip)
cyclist_vandix_irr_adjusted <- inla.posterior.sample(1000, m5_cyclist_zip)
...
vandix_irr_cyclist_crashes_models <- left_join(cyclist_vandix_irr_unadjusted,cyclist_vandix_irr_minimally_adjusted,by="region") %>% 
...
model_list_pedestrian <- list(m1_pedestrian,m1_pedestrian_zip,m2_pedestrian,m2_pedestrian_zip,m3_pedestrian, m3_pedestrian_zip)
model_names <- c("Poisson",
                 "ZI Poisson",
                 "Poisson - Unstructured RE",
                 "ZI Poisson - Unstructured RE",
...
pedestrian_crashes_full_model_details <- summarise_models_details(inla_model_list = model_list_pedestrian,inla_model_names = model_names )


pedestrian_crashes_full_model_details %>%  
  filter(str_detect(term,"WAIC|DIC")) %>% 
  select(term,contains("mean")) %>% 
  pivot_longer(cols = contains("Poisson")) %>% 
  pivot_wider(id_cols = name,names_from = term,values_from = value) %>% 
...
f4_pedestrian <-  n_pedestrian_casualty_claims ~ vandix_z*region + ln_roads_km_c + f(ui, model = 'bym2', graph = cma_adj, hyper = prior, adjust.for.con.comp = TRUE, constr = TRUE, scale.model = TRUE)

m4_pedestrian_zip <- inla(f4_pedestrian,
           data =  cma_sp@data,
           family = "zeroinflatedpoisson1",
           control.compute = list(dic = TRUE, waic = TRUE,config=TRUE),
           control.predictor = list(compute = TRUE),
...
f5_pedestrian <-  n_pedestrian_casualty_claims ~ vandix_z*region + ln_roads_km_c +  roads_prop_highway_arterial_z +  bike_infra_cat + population_100_c + number_intersections_10_c  + f(ui, model = 'bym2', graph = cma_adj, hyper = prior, adjust.for.con.comp = TRUE, constr = TRUE, scale.model = TRUE)

m5_pedestrian_zip <- inla(f5_pedestrian,
           data =  cma_sp@data,
           family = "zeroinflatedpoisson1",
           control.compute = list(dic = TRUE, waic = TRUE,config=TRUE),
           control.predictor = list(compute = TRUE),
...
######### Region Specific IRRs from Posterior Distribution of Vandix + Region Interaction
pedestrian_vandix_irr_unadjusted <- inla.posterior.sample(1000, m3_pedestrian_zip)
pedestrian_vandix_irr_minimally_adjusted <- inla.posterior.sample(1000, m4_pedestrian_zip)
pedestrian_vandix_irr_adjusted <- inla.posterior.sample(1000, m5_pedestrian_zip)
...
vandix_irr_pedestrian_crashes_models <- left_join(pedestrian_vandix_irr_unadjusted,pedestrian_vandix_irr_minimally_adjusted,by="region") %>% 
