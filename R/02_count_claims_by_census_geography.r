...
total_crashes_by_da <- claims_rnd_assignment_duplicates %>%
  group_by(GeoUID) %>%                 # Group by the geographic unit ID
  summarise(
...
bc_da <- bc_da %>%
  left_join(total_crashes_by_da, by = "GeoUID") %>%  # Add crash data to the census data
  replace_na(
    list(
      n_claims = 0,
      n_casualty_claims = 0,
...
st_write(bc_da, dsn = paste0(wd, "/Processed Data/da_v2_2021.gpkg"))
