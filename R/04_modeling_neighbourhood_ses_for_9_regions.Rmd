...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10,warning=FALSE,message=FALSE,fig.cap="Regional Grouping of Census Metropolitan and Agglomeration Areas. Labelled Points Represent Census Metroplitan Area and Census Agglomeration Centroids"}
...
bc_da %>%
  ...
  summarise(
    ...
    population = sum(population, na.rm = TRUE),
    # Population with thousands separator and percentage
    `Population (%)` = paste0(
      format(sum(population, na.rm = TRUE), big.mark = ","),
      " (", signif(100 * sum(population, na.rm = TRUE) / sum(bc_da$population, na.rm = TRUE), 3), "%)"
    ),
    
    # Total Roads (km) with thousands separator and percentage
    `Total Roads (km) (%)` = paste0(
      format(ceiling(sum(total_roads_km, na.rm = TRUE)), big.mark = ","),
      " (", signif(100 * sum(total_roads_km, na.rm = TRUE) / sum(bc_da$total_roads_km, na.rm = TRUE), 3), "%)"
    ),
    
        # Mean and SD for other variables
    `Percentage of Major Roads (SD)` = paste0(
      signif(mean(roads_prop_highway_arterial*100, na.rm = TRUE), 3), 
      " (", signif(sd(roads_prop_highway_arterial*100, na.rm = TRUE), 3), ")"
    ),
    
    # Total Injury Claims with thousands separator and percentage
    `Total Injury Claims (%)` = paste0(
      format(sum(n_casualty_claims), big.mark = ","),
      " (", signif(100 * sum(n_casualty_claims) / sum(bc_da$n_casualty_claims),3), "%)"
    ),
    ...
    # Total Cycling Injury Claims with thousands separator and percentage
    `Total Cycling Injury Claims (%)` = paste0(
      format(sum(n_cyclist_casualty_claims), big.mark = ","),
      " (", signif(100 * sum(n_cyclist_casualty_claims) / sum(bc_da$n_cyclist_casualty_claims), 3), "%)"
    ),
    ...
    # Total Pedestrian Injury Claims with thousands separator and percentage
    `Total Pedestrian Injury Claims (%)` = paste0(
      format(sum(n_pedestrian_casualty_claims), big.mark = ","),
      " (", signif(100 * sum(n_pedestrian_casualty_claims) / sum(bc_da$n_pedestrian_casualty_claims), 3), "%)"
    ),
...
all <- all %>%
  ...
  select(Variable,Region,Outcome, everything()) %>%
  split(.$Outcome)
```
...
lr_scatter <- function(outcome, title, subtitle) {
  # Create the plot
  bc_da %>%
    st_drop_geometry() %>%
    filter(!is.na(region_name)) %>%
    mutate(region_name = factor(region_name, levels = regions$region_name)) %>%
    ggplot(aes(x = vandix_z, y = {{outcome}})) +
    ...
    labs(
      title = title,
      subtitle = subtitle,
      x = "VanDIX (Increasing Deprivation)",
...
crude_rates_all <- lr_scatter(casualty_claims_rate, title = "All Injury Crash Rates by Neighbourhood Deprivation", subtitle = "At Dissemination Area Level")

ggsave(crude_rates_all,
  filename = "C:/Users/micha/Documents/GitHub/Area-Level-Deprivation-Traffic-Injury/Figures/crude_rates_all.jpeg",
  units = "mm",
  height =280,
  width =  280,
  dpi = 300
...
crude_rates_cyclists <- lr_scatter(cyclist_casualty_claims_rate, title = "Cyclist Involved Injury Crash Rates by Neighbourhood Deprivation", subtitle = "At Dissemination Area Level")

ggsave(crude_rates_cyclists,
  filename = "C:/Users/micha/Documents/GitHub/Area-Level-Deprivation-Traffic-Injury/Figures/crude_rates_bicycle.jpeg",
  units = "mm",
  height =280,
  width =  280,
  dpi = 300
...
crude_rates_pedestrians <- lr_scatter(pedestrian_casualty_claims_rate, title = "Pedestrian Involved Injury Crash Rates by Neighbourhood Deprivation", subtitle = "At Dissemination Area Level")

ggsave(crude_rates_pedestrians,
  filename = "C:/Users/micha/Documents/GitHub/Area-Level-Deprivation-Traffic-Injury/Figures/crude_rates_pedestrians.jpeg",
  units = "mm",
  height =280,
  width =  280,
  dpi = 300
)
crude_rates_pedestrians
```
...
irr_table <- function(table,caption) {
  table %>%
    select(-Outcome, -variable) %>%
    flextable() %>%
    add_footer_lines(values = "SD = Standard Deviation; IRR = Incidence Rate Ratio; CI = Credible Interval") %>% 
    footnote(
      i = c(1),
      j = c(3, 4, 5, 6),
      value = as_paragraph(
        c("Fit with idd random effect at dissemination area level",
          ...
          "Fit with idd and spatial random effect at dissemination area level + adjusted for (i) total length of roads; (ii) proportion of roads classified as highway or arterial; (iii) the Canadian Active Living Environment Index; (iv) the Canadian Bikeway Comfort and Safety Index; (v) total population"
        )
      ),
...
forest_plot <- function(table,title,subtitle){
  
  
table %>% 
    left_join(region_pop, by = c("Region"="region_name")) %>% 
    mutate(`Population (1000s)` = population/1000) %>% 
  mutate(Adjusted_IRR = gsub(" ", "", `Spatial Adjusted IRR (95% CI)`), # Remove spaces
         estimate = as.numeric(gsub("\\(.*", "", `Spatial Adjusted IRR (95% CI)`)),
         lower_ci = as.numeric(gsub(".*\\(([^,]*),.*", "\\1", `Spatial Adjusted IRR (95% CI)`)),
         upper_ci = as.numeric(gsub(".*,(.*)\\)", "\\1", `Spatial Adjusted IRR (95% CI)`))) %>% 
ggplot(aes(x = fct_reorder(Region, estimate), y = estimate)) +
  geom_linerange(aes(ymin = lower_ci, ymax = upper_ci)) + 
      geom_point(aes(size = `Population (1000s)`)) +  
  ...
  hrbrthemes::theme_ipsum(axis = "xy",grid = "Xx",ticks = TRUE,base_size = 12,axis_title_size = 11)
}

irr_table(all$`All Injury Claims`,"Regional associations between VanDIX score and number of motor-vehicle crash insurance claims from 2019-2023 fit with nonspatial and spatial models and different covariate values.")

forest_plot(all$`All Injury Claims`,title = "Neighborhood Deprivation and All Road Traffic Injury Crashes by Region",  subtitle = "Each standard deviation increase in VanDIX increases the incidence road traffic injury crashes in all regions (IRR>1)")
...
irr_table(all$`Cyclist Injury Claims`,"Regional associations between VanDIX score and number of motor-vehicle crash insurance claims involving a cyclist from 2019-2023 fit with nonspatial and spatial models and different covariate values.")


forest_plot(all$`Cyclist Injury Claims`,title = "Neighborhood Deprivation and All Road Traffic Injury Crashes by Region",  subtitle = "Each standard deviation increase in VanDIX increases the incidence road traffic injury crashes in nearly all regions (IRR>1) with much wider credible intervals")
...
irr_table(all$`Pedestrian Injury Claims`,"Regional associations between VanDIX score and number of motor-vehicle crash insurance claims involving a pedestrian from 2019-2023 fit with nonspatial and spatial models and different covariate values.")

forest_plot(all$`Pedestrian Injury Claims`,title = "Neighborhood Deprivation and All Road Traffic Injury Crashes by Region",  subtitle = "Each standard deviation increase in VanDIX increases the incidence road traffic injury crashes in  all regions (IRR>1)")
```
...
#### All
```{r, echo = FALSE, message = FALSE, fig.width= 7.5, fig.height=10}
...
results_hyper_parameters_combined <- reduce(results_hyper_parameters, full_join, by = "term") %>% 
        mutate(
      `Nonspatial` = ifelse(is.na(mean.x), "", paste0(
        prs(mean.x),
        " (",
        prs(`0.025quant.x`),
        ", ",
      ...
      `Unadjusted` = ifelse(is.na(mean.y), "", paste0(
        prs(mean.y),
        " (",
        prs(`0.025quant.y`),
        ", ",
      ...
      `Adjusted1` = ifelse(is.na(mean.x.x), "",paste0(
        prs(mean.x.x),
        " (",
        prs(`0.025quant.x.x`),
        ", ",
    ...
    select(term,`Nonspatial`,`Unadjusted`,`Adjusted1`,`Adjusted2`)
# Applying function to each unique name
results_combined <- reduce(results, full_join, by = "term") %>% 
        mutate(
      `Nonspatial` = ifelse(is.na(estimate.x), "", paste0(
        prs(estimate.x),
        " (",
        prs(`lower_ci.x`),
        ", ",
      ...
      `Unadjusted` = ifelse(is.na(estimate.y), "", paste0(
        prs(estimate.y),
        " (",
        prs(`lower_ci.y`),
        ", ",
      ...
      `Adjusted1` = ifelse(is.na(estimate.x.x), "",paste0(
        prs(estimate.x.x),
        " (",
        prs(`lower_ci.x.x`),
        ", ",
...
extract_metrics <- function(model) {
  tibble(
    term = c("WAIC","CPO","DIC"),
    estimate = c(model$waic$waic,
                 sum(log(model$cpo$cpo)) * -1,#CPO is a cross-validatory criterion for model assessment that where the value for each observation is the posterior probability of observing that observation when the model is fit using all data except for itself.  Large values indicate a better fit of the model to the data. A summary measure for CPO where smaller values indicate better fit. 
                 model$dic$dic)
...
model_comparison_combined <- inla_model_list %>%
  map(extract_metrics) %>% 
  map(~mutate(.,estimate = round(estimate,1))) %>% 
  reduce(., full_join, by = "term") %>% 
  rename( Nonspatial = estimate.x, Unadjusted = estimate.y, Adjusted1 = estimate.x.x, Adjusted2 = estimate.y.y) %>% 
...
result_tibbles <- bind_rows(results_combined,results_hyper_parameters_combined,model_comparison_combined) %>% 
    mutate( 
      Nonspatial = str_remove_all(Nonspatial,"NA \\(|NA\\,|NA\\)"),
      Unadjusted = str_remove_all(Unadjusted,"NA \\(|NA\\,|NA\\)"),
...
vancouver_sf <- st_transform(vancouver_sf,crs=26910)
vancouver_sf$re_all_adjusted <- van_models_1$models$Adjusted2$summary.random$ui[1:nrow(vancouver_sf), "mean"]

plot_re <- function(sf,fill_var, subtitle) {
ggplot() + 
  geom_sf(data = bcb %>% st_transform(st_crs(sf)), fill = "grey70", colour = "black") +
  geom_sf(data = sf, aes(fill = {{fill_var}})) +
  ...
  theme(legend.position = "bottom",
            panel.background = element_rect(fill = "#a3c1e0"),
        panel.border = element_rect(colour="black",fill = NA)# Light blue ocean color
) +
  labs(
    title = "Posterior mean of the BYM2 random effect", 
    subtitle = subtitle
  ) + 
  coord_sf(
    xlim = c(st_bbox(sf)["xmin"], st_bbox(sf)["xmax"]),
    ylim = c(st_bbox(sf)["ymin"], st_bbox(sf)["ymax"])
  ) + 
  ggspatial::annotation_scale(location = "bl", width_hint = 0.25,)  # 'bl' = bottom left
}
...
summarise_models_details(van_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Vancouver Region")

plot_re(vancouver_sf,re_all_adjusted,"Adjusted Model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(van_models_2$models) %>% flextable_model_details(., caption = "Cyclist - Motor Vehicle Claims for Vancouver Region")

vancouver_sf$re_cyclists_adjusted <- van_models_2$models$Adjusted2$summary.random$ui[1:nrow(vancouver_sf), "mean"]

plot_re(vancouver_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(van_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for Vancouver Region")

vancouver_sf$re_pedestrians_adjusted <- van_models_3$models$Adjusted2$summary.random$ui[1:nrow(vancouver_sf), "mean"]

plot_re(vancouver_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(victoria_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Victoria Region")



victoria_sf <- st_transform(victoria_sf,crs=26910)
victoria_sf$re_all_adjusted <- victoria_models_1$models$Adjusted2$summary.random$ui[1:nrow(victoria_sf), "mean"]


plot_re(victoria_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(victoria_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for Victoria Region")


victoria_sf$re_cyclists_adjusted <- victoria_models_2$models$Adjusted2$summary.random$ui[1:nrow(victoria_sf), "mean"]

plot_re(victoria_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(victoria_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for Victoria Region")

victoria_sf$re_pedestrians_adjusted <- victoria_models_3$models$Adjusted2$summary.random$ui[1:nrow(victoria_sf), "mean"]

plot_re(victoria_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(nanaimo_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Central Island - Powell River Region")



nanaimo_sf <- st_transform(nanaimo_sf,crs=26910)
nanaimo_sf$re_all_adjusted <- nanaimo_models_1$models$Adjusted2$summary.random$ui[1:nrow(nanaimo_sf), "mean"]


plot_re(nanaimo_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(nanaimo_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for Central Island - Powell River Region")


nanaimo_sf$re_cyclists_adjusted <- nanaimo_models_2$models$Adjusted2$summary.random$ui[1:nrow(nanaimo_sf), "mean"]

plot_re(nanaimo_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(nanaimo_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for Central Island - Powell River Region")

nanaimo_sf$re_pedestrians_adjusted <- nanaimo_models_3$models$Adjusted2$summary.random$ui[1:nrow(nanaimo_sf), "mean"]

plot_re(nanaimo_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(okanagan_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Okanagan Region")



okanagan_sf <- st_transform(okanagan_sf,crs=26910)
okanagan_sf$re_all_adjusted <- okanagan_models_1$models$Adjusted2$summary.random$ui[1:nrow(okanagan_sf), "mean"]


plot_re(okanagan_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(okanagan_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for Okanagan Region")


okanagan_sf$re_cyclists_adjusted <- okanagan_models_2$models$Adjusted2$summary.random$ui[1:nrow(okanagan_sf), "mean"]

plot_re(okanagan_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(okanagan_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for Okanagan Region")

okanagan_sf$re_pedestrians_adjusted <- okanagan_models_3$models$Adjusted2$summary.random$ui[1:nrow(okanagan_sf), "mean"]

plot_re(okanagan_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(fraser_valley_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Fraser Valley Region")



fraser_valley_sf <- st_transform(fraser_valley_sf,crs=26910)
fraser_valley_sf$re_all_adjusted <- fraser_valley_models_1$models$Adjusted2$summary.random$ui[1:nrow(fraser_valley_sf), "mean"]


plot_re(fraser_valley_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(fraser_valley_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for Fraser Valley Region")
...
plot_re(fraser_valley_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(fraser_valley_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for  Fraser Valley Region")
...
plot_re(fraser_valley_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(north_central_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for North Central Region")



north_central_sf <- st_transform(north_central_sf,crs=26910)
north_central_sf$re_all_adjusted <- north_central_models_1$models$Adjusted2$summary.random$ui[1:nrow(north_central_sf), "mean"]


plot_re(north_central_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(north_central_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for North Central Region")
...
plot_re(north_central_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(north_central_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for North Central Region")
...
plot_re(north_central_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(interior_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Kamloops-Salmon Arm Region")



interior_sf <- st_transform(interior_sf,crs=26910)
interior_sf$re_all_adjusted <- interior_models_1$models$Adjusted2$summary.random$ui[1:nrow(interior_sf), "mean"]


plot_re(interior_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(interior_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for Kamloops-Salmon Arm Region")


interior_sf$re_cyclists_adjusted <- interior_models_2$models$Adjusted2$summary.random$ui[1:nrow(interior_sf), "mean"]

plot_re(interior_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(interior_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for Kamloops-Salmon Arm Region")

interior_sf$re_pedestrians_adjusted <- interior_models_3$models$Adjusted2$summary.random$ui[1:nrow(interior_sf), "mean"]

plot_re(interior_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(southeast_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Southeast Region")



southeast_sf <- st_transform(southeast_sf,crs=26910)
southeast_sf$re_all_adjusted <- southeast_models_1$models$Adjusted2$summary.random$ui[1:nrow(southeast_sf), "mean"]


plot_re(southeast_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(southeast_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for SoutheastRegion")


southeast_sf$re_cyclists_adjusted <- southeast_models_2$models$Adjusted2$summary.random$ui[1:nrow(southeast_sf), "mean"]

plot_re(southeast_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(southeast_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for Southeast Region")

southeast_sf$re_pedestrians_adjusted <- southeast_models_3$models$Adjusted2$summary.random$ui[1:nrow(southeast_sf), "mean"]

plot_re(southeast_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
#### All
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(northwest_models_1$models) %>% flextable_model_details(., caption = "All Motor Vehicle Claims for Northwest Region")



northwest_sf <- st_transform(northwest_sf,crs=26910)
northwest_sf$re_all_adjusted <- northwest_models_1$models$Adjusted2$summary.random$ui[1:nrow(northwest_sf), "mean"]


plot_re(northwest_sf,re_all_adjusted,"Adjusted model for all Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(northwest_models_2$models) %>% flextable_model_details(., caption = "Cyclist- Motor Vehicle Claims for NorthwestRegion")


northwest_sf$re_cyclists_adjusted <- northwest_models_2$models$Adjusted2$summary.random$ui[1:nrow(northwest_sf), "mean"]

plot_re(northwest_sf,re_cyclists_adjusted,"Adjusted Model for Cyclist - Motor Vehicle Claims")
...
```{r, echo = FALSE, fig.width= 7.5, fig.height=10}
summarise_models_details(northwest_models_3$models) %>% flextable_model_details(., caption = "Pedestrian - Motor Vehicle Claims for Northwest Region")

northwest_sf$re_pedestrians_adjusted <- northwest_models_3$models$Adjusted2$summary.random$ui[1:nrow(northwest_sf), "mean"]

plot_re(northwest_sf,re_pedestrians_adjusted,"Adjusted Model for Pedestrian - Motor Vehicle Claims")
...
bc_da %>%
  ...
  labs(
    title = "Proportion of Roads that are Arterial or Highway by Neighbourhood Deprivation",
    x = "VanDIX (Increasing Deprivation)",
...
bc_da %>%
  ...
  facet_wrap(~region_name) +
  labs(
    title = "Active Living Environment Index by Neighbourhood Deprivation",
    x = "VanDIX (Increasing Deprivation)",
    y = "ALE Index (SD) "
  ) +
  theme_minimal() +
...
bc_da %>%
  ...
  facet_wrap(~region_name) +
  labs(
    title = "Canadian Bikeway Comfort Index by Neighbourhood Deprivation",
    x = "VanDIX (Increasing Deprivation)",
    y = "CanBICS Index (SD) "
  ) +
  theme_minimal() +
...
<!-- BYM2 has two hyperparameters, a precision and mixing parameter. The precision parameter controls the variability explained by a spatial effect. The mixing parameter distributes existing variability between unstructured ($u$) and structured model components($v$). The mixing parameter prior was set to (U = 0.5,α = 2/3), which assumes the unstructured random effect accounts for more of the variability than the spatially structured effect. The precision parameter was set to (U = 0.2/0.31, α = 0.01). The BYM2 model is equal to an only spatial model when the mixing parameter is equal to 1 and and an only unstructured spatial noise when it's 0. -->
