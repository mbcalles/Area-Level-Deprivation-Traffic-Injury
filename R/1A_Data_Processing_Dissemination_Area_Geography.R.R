# -----------------------------------------------------------------------------
# Script Name: 1A_Data_Processing_Dissemination_Area_Geography.R
...
bc_da <- bc_da %>% #Calculate VanDIX score
...
roads_agg_da <- roads_clip_da %>% 
  map(.,~st_join(.,bc_da_buff, join = st_intersects) %>% #assign the DAUID that overlaps with a given line segment 
        st_drop_geometry() %>% 
        group_by(GeoUID) %>% 
...
claims_da_join <- st_join(claims_sf,bc_da_buff,join = st_intersects) %>% as_tibble()
...
total_pedestrian_crashes_by_da <- claims_da_join %>% 
